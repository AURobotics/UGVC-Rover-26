...
class FaceRecognition:
    ...
    def recognize_frame(self, frame):
        ...
        if faces is not None:
            for face in faces:
                ...
                if score > 0.45:
                    ...
                    color = (0, 255, 0)
                    is_detected = True                        
                    offset_x = int(self.get_offset(face, frame))
                else:
                    label = "UNKNOWN"
                    color = (0, 0, 255)
        ...
        return frame, is_faces, is_detected, offset_x

    def get_offset(self, face, frame):
        frame_width = frame.shape[1]
        x = int(face[0])
        w = int(face[2])
        face_center_x = x + w // 2
        frame_center_x = frame_width // 2
        return face_center_x - frame_center_x
...
def main():
    ...
    while True:
        ...
        frame, is_faces, is_detected, offset = face_recognition.recognize_frame(frame)
        print(f"Faces Detected: {is_faces}, Target Detected: {is_detected}, Offset X: {offset}px")
